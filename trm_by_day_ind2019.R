...
TrmByDayInd <- function(x){
  if (ncol(x) == 23) {
    trm.con  <- as.numeric(x[9,  2:23])
...
by.day.data <- lapply(file.list, TrmByDayInd)
...
by.day.data -> trm.daily.ind2019
rm(list = setdiff(ls(), "trm.daily.ind2019"))

# Сохранение результатов -----------------------------------------------
save(trm.daily.ind2019, file = "~/data/trm_daily_ind2019")
